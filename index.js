...
app.get('/api/websites/all', async (req, res, next) => {
  try {
    const result = await Websites.find().limit(5).select().exec();
...
const getscreenshot = async (data, size) => {
  // const imgpath = path.join(public, data.url);

  const browser = await puppeteer.launch( {"headless": false, args: [`--window-size=${size}`], defaultViewport: null });
  ...
  await page.screenshot({
    path: __dirname + '/public/' +  data.name +  size.replace(',', 'x') +  '.png',
    //fullPage: true
  }); 
  await browser.close();
